...
class AuthorityLogger:
    ...
    def start_xterm(self) -> None:
        ...
        xterm_cmd = [
            'xterm',
            '-title', f'{self.authority_name} Authority Log',
            '-geometry', '100x30',
            '-bg', 'black',
            '-fg', 'green',
        ...
        try:
            ...
            print(f"Opened xterm for {self.authority_name} (PID: {self.terminal_process.pid})")
        except Exception as e:
            print(f"Failed to open xterm for {self.authority_name}: {e}")
