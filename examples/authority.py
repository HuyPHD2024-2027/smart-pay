...
class TransferTestClient:
    ...
    def connect(self, authority: Station) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Connecting to authority {authority.name} at {auth_ip}")
    ...
    def send_transfer_order(self, sender: str, recipient: str, amount: int) -> bool:
        ...
        try:
            ...
            # Send transfer order
            message = json.dumps(transfer_order).encode('utf-8')
            self.socket.sendall(message)
            
            self.logger.sent("Transfer order sent successfully")
            return True
            
        except Exception as e:
            self.logger.error(f"Failed to send transfer order: {e}")
            return False
...
def open_authority_xterms(authorities):
    ...
    for authority in authorities:
        if hasattr(authority, 'logger'):
            ...
            authority.logger.start_terminal()
...
def test_transfer_order(net, authorities, stations):
    ...
    def send_transfer(sender, recipient, amount):
        """Send transfer between users."""
        info(f"*** Processing transfer: {sender} → {recipient}, {amount} tokens\n")
        
        test_client = TransferTestClient("CLI-Client")
        successful_responses = 0
        total_authorities = len(authorities)
        
        # Test each authority
        for authority in authorities:
            authority.logger.info(f"Processing transfer via {authority.name}")
            
            # Connect to authority
            if test_client.connect(authority):
                # Send transfer order
                if test_client.send_transfer_order(sender, recipient, amount):
                    # Wait for response
                    response = test_client.receive_response()
                    if response and response.get('success'):
                        successful_responses += 1
                        authority.logger.success("Transfer approved")
                    else:
                        authority.logger.error("Transfer rejected")
                test_client.disconnect()
            else:
                authority.logger.error("Connection failed")
        
        # Show results
        consensus_threshold = (total_authorities * 2) // 3 + 1  # 2/3 + 1 majority
        info(f"*** Transfer Results: {successful_responses}/{total_authorities} approvals\n")
        info(f"*** Consensus threshold: {consensus_threshold}\n")
        
        if successful_responses >= consensus_threshold:
            info("*** ✅ TRANSFER CONFIRMED: Consensus achieved!\n")
        else:
            info("*** ❌ TRANSFER REJECTED: Consensus failed!\n")
